  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const orderedOrders = orders.filter(order => order.OrderType === 'reviewed');
    ...
    { field: 'createdBy', headerName: 'Created By', width: 150 },
    {
      ...
      renderCell: (params) => (
        <Button
          variant="contained"
          color="primary"
          onClick={() => navigate(`/reviewed_overview/${params.row.id}`)}
        >
          View
        </Button>
      ),

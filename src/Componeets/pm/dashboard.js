...
function Dashboard() {
  ...
  const [totalCommissioned, setTotalCommissioned] = useState(0);
  ...
  useEffect(() => {
    const fetchOrderCounts = async () => {
      try {
        ...
        const commissionedResponse = await axios.get('http://localhost:3000/categories/totalcommission', { headers });
        setTotalCommissioned(commissionedResponse.data.subtotal);
  ...
  <Link to="product" style={{ textDecoration: 'none' }}>
    ...
    >
      ...
      />
      <CardContent>
        <Typography variant="h5" sx={{ color: '#333' }}>{totalCommissioned}</Typography>
      </CardContent>
    </Card>
  </Link>
</Grid>

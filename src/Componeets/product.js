...
const Products = () => {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      const sellerIdMatch = searchParams.sellerId === '' || (product.sellerid && product.sellerid.toString().includes(searchParams.sellerId));
  ...
  const columns = [
    ...
    { field: 'sellerid', headerName: 'Seller ID', width: 100 },
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        return (
          <Button
            variant="contained"
            color={isReserved ? 'info' : 'secondary'} 
            disabled={isReserved}
